...
if __name__ == "__main__":
    channel = input(
        'Type a username of a Telegram channel to get similar channels: ')
    ...
    with open(f'{channel}.csv', 'w', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'username', 'title',
                        'verified', 'participants_count'])
        for c in result.chats:
            usernames = '|'.join(
                [u.username for u in c.usernames]) if c.usernames else c.username
            writer.writerow([c.id, usernames, c.title,
                            c.verified, c.participants_count])
